  ...
  describe '.create_or_update' do
    context 'when history does not exist' do
      ...
      before do
        ...
        allow(dynamodb_client).to receive(:get_item).and_return(instance_double(Aws::DynamoDB::Types::GetItemOutput, item: nil))
        allow(dynamodb_client).to receive(:put_item)
      end

      it 'creates a new history' do
        described_class.create_or_update(user_id, product_name, viewed_at)
